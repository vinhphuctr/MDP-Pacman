...
public class MazePacman {
    ...
    public MazePacman(String filename) throws MazeException {
        try {
            ...
            for (int x = 0; x < size_x; x++)
                if (!walls[x][0]) throw new MazeException("Wrong input format: the maze must be closed");
            for (int x = 0; x < size_x; x++)
                if (!walls[x][size_y - 1]) throw new MazeException("Wrong input format: the maze must be closed");
            for (y = 0; y < size_y; y++)
                if (!walls[0][y]) throw new MazeException("Wrong input format: the maze must be closed");
            for (y = 0; y < size_y; y++)
                if (!walls[size_x - 1][y]) throw new MazeException("Wrong input format: the maze must be closed");
    ...
    public void setSizeY(int x) {
        size_x = x;
    }

    public void addPacmanStart(int x, int y) {
        pacman_start_x.add(x);
        pacman_start_y.add(y);
    }

    public void addGhostStart(int x, int y) {
        ghosts_start_x.add(x);
        ghosts_start_x.add(y);
    }
